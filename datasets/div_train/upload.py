...
def merge_common_columns_drop_top_index(path_a: str, path_b: str) -> pa.Table:
    ...
    # Drop problematic columns to avoid HF key/conflicts and nested schema issues
    for drop_col in ["index", "extra_info"]:
        if drop_col in common_cols:
            common_cols.remove(drop_col)

    if not common_cols:
        raise ValueError("No common columns found between input Parquet files after dropping 'index'.")
...
def main():
    ...
    table = merge_common_columns_drop_top_index(src_a, src_b)
